<div class=border-1>
  <Player bind:audio={audio_el} timeupdate={time_updated} {src}/>
</div>

<fieldset class="p-1 mt-4">
  <label class=block>Audio file <input type=text bind:value={src} on:input={set_src} class="block w-20"></label>
  <div class=flex>
...
// let src = 'https://files.nuqayah.com/timing/1-cbr.mp3'
let src = '#'
let audio_el
let times = []
let current_s_i = 0
...
;(async () => {
    src = await kv.get('src') || 'https://data.sunnati.net/files/jami-alsahihayn/wird-01.mp3'
...
async function set_src(e) {
    await kv.set('src', e.target.value)
}
